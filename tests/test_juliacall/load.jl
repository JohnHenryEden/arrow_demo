...
# 1) 定义 keys(lp) → 返回字段名 Tuple
Base.keys(lp::LPproblem) = (
    :S, :b, :c, :lb, :ub, :osense, :csense, :rxns, :mets
)

# 2) 定义 getindex(lp, key::Symbol) → 用 property 访问
function Base.getindex(lp::LPproblem, key::Symbol)
    if hasproperty(lp, key)
        return getproperty(lp, key)
    else
        throw(KeyError(key))
    end
end
